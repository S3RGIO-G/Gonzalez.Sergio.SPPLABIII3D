# RecusApp

## Integrantes: 
`"Alfa"`: Orce Patricio <br>
`"Beta"`: Ovelar Mariano <br>
`"Gamma"`: Gonzalez Sergio <br>

## Iniciar Sesión:
<div align="center">
<img src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2FScreenshot_20231118-093515.png?alt=media&token=1111ed7c-b904-4665-872b-2c7e8a03dbbf" width=220 height=475 >
<img src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2FScreenshot_20231118-093554.png?alt=media&token=e6945216-1d5b-48a7-ac03-b76ba1c8cf8d" width=220 height=475 >
</div>

Aqui podrás iniciar sesion en caso de que tengas cuenta, o redirigirte `Registrar cuenta` en caso de que no.
<br>
Tambien podras iniciar sesión como **Anonimo** si deseas usar nuestra aplicación sin registrarte.
<br>
<br>
**Ver usuarios:**
<br>
Este botón despliega un modal con distintos usuarios precargados para que puedas probar la aplicacion con todos sus roles

## Registrar cuenta:

<div align="center"> 
<img src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2FScreenshot_20231118-093537.png?alt=media&token=b836bdd9-8419-4e09-8d25-8308ce018334" width="220" height="475">
</div>

Te permite crear una cuenta como cliente, la cual debera ser aprobada por un **`Dueño`** o **`Supervisor`**
<br>
Puedes escanear el codigo QR de un **DNI** o cargar los datos manualmente.

## Rechazado o Pendiente:

<div align="center">
  <img align="center"
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fpending.jpeg?alt=media&token=182b6f61-7119-451e-a845-f9da7c7463bc"
    width="220" height="475">
  <img align="center"
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Freject.jpeg?alt=media&token=5586a140-d697-4643-ae27-23a879b57aa7"
    width="220" height="475">
</div>

En caso de `Iniciar sesión` y que tu solicitud aún no haya sido **aprobada** o haya sido **rechazada**, veras la pantalla correspondiente a tu situación.

## Dueño o Supervisor:

<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fowner%2FScreenshot_20231118-093613.png?alt=media&token=0e6c0eed-dfda-4c99-86ea-873ffb963c38"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fowner%2FScreenshot_20231118-094539.png?alt=media&token=f059b43a-af16-4cf4-9ce7-9692654cfc5a"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fowner%2Fmesas.jpeg?alt=media&token=f9e3bfc5-6cf1-4278-83ab-12343d921a64"
    width="220" height="475">
</div>

Puedes acceder al listado solicitudes de registro pedientes, las cuales puedes **rechazar** o **aceptar**.
<br>
Además, puedes ver el listado de mesas existentes con su estado actual o visitar el listado de productos del `Menu`.
<br>

## Menu:

<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmenu%2FScreenshot_20231118-101544.png?alt=media&token=94f98c20-993d-48a4-ad7f-ef4d7d9a2b01"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmenu%2FScreenshot_20231118-101539.png?alt=media&token=c4963683-efa9-4f3c-b729-120790c4d474"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmenu%2FScreenshot_20231118-101551.png?alt=media&token=6535a762-d931-496e-b7f9-bf8e3aa68e87"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmenu%2FScreenshot_20231118-101600.png?alt=media&token=8ee91063-679e-468e-85ac-019db31ea231"
    width="220" height="475">
</div>

Si la ruta recibe el `QueryParam` **"buy=false"** solo podrás ver el listado de productos disponibles.
<br>
En cambio, si recibe **"buy=true"** podrás armar tu pedido con los productos disponibles.
<br>
<br>
**Ver pedido:**
<br>
Despliega un modal en el que veras los detalles de tu pedido, ademas de poder modificarlos.
<br>
<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmenu%2FScreenshot_20231118-101648.png?alt=media&token=e0cd1437-ff37-4656-a8ac-ce7b7e739c69"
    width="220" height="475">
</div>

## Cocinero o Bartender :

<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcocinero%26bartender%2FScreenshot_20231118-094630.png?alt=media&token=54b2125b-f6ee-4fcd-bc10-720323a4ab15"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcocinero%26bartender%2FScreenshot_20231118-094857.png?alt=media&token=90d39d58-3451-4cf4-90cc-403f609022a8"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcocinero%26bartender%2FScreenshot_20231118-094906.png?alt=media&token=a3cf8934-b683-44c3-a89c-c1cf55ecca16"
    width="220" height="475">
</div>

Se tendrá acceso al listado de **pedidos pendientes**, de acuerdo al tipo de empleado, para prepararlos.

## Metre:

<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmetre%2FScreenshot_20231118-094235.png?alt=media&token=b6ef7fae-536b-4e1c-bd20-0daf720f9440"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmetre%2Fmesas-metre.jpeg?alt=media&token=4cc71beb-b4da-4c5c-b779-cf2adbe70dfe"
    width="220" height="475">
</div>

Puedes ver el listado de usuarios que estan esperando y asignarles una mesa que esté **LIBRE**

## Mozo:
<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmozo%2FScreenshot_20231118-100214.png?alt=media&token=07225c00-fc02-4288-b232-98ebd760ed7a"
    width="220" height="475">
</div>

Acá va a podes `confirmar` pedidos, hacer el seguimiento y hacer su `entrega` unas vez esté listo.
<br>
Además, vas a poder acceder al `Chat` de la mesa que quieras, y `Cobrar` la cuenta de las mesas que hayan sido **pagadas**.

### Ver pedidos:
<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmozo%2FScreenshot_20231118-100227.png?alt=media&token=4633c751-a397-41a2-ad42-e30319c9a7d4"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmozo%2FScreenshot_20231118-100240.png?alt=media&token=548536cf-3ed5-480d-88a9-d38ea1cb24ae"
    width="220" height="475">
</div>

Puedes seleccionar el pedido que desees confirmar, el estado del pedido pasará a **pendiente** y quedará asignado al `Mozo`, por lo que podrá hacer su seguimiento en `"Mis pedidos"`.
### Mis pedidos:
<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmozo%2FScreenshot_20231118-100341.png?alt=media&token=0ec56305-c34a-4f65-8a72-01ba2842e393"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmozo%2FScreenshot_20231118-100349.png?alt=media&token=3b310da5-e121-46d0-a55c-2cf34f4af2e3"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmozo%2FScreenshot_20231118-100453.png?alt=media&token=661e1d7c-381b-4001-8316-d6fc4499ad69"
    width="220" height="475">
</div>

Puedes hacer el seguimiento del pedido que previamente **confirmaste**, y `entregarlo` una vez este completamente **listo**.

### Cobrar mesa:
<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmozo%2FScreenshot_20231118-100831.png?alt=media&token=295fd2f7-9442-4ebd-9d94-efefe5772353"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fmozo%2FScreenshot_20231118-100812.png?alt=media&token=7b9f01dc-6b95-4a81-aef2-49a94853690f"
    width="220" height="475">
</div>

Al seleccionar una mesa que haya sido `pagada`, podrás ver un detalle de la cuenta, mostrando la **propina**, el **descuento** y su **total**.
<br>
Cuando se `confirma el pago`, la mesa se libera automaticamente.

### Chats: 
Despliega un modal similar al de `Cobrar mesa`, solo que al seleccionar una mesa, te redirige al chat correspondiente.

## Cliente:
<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-101249.png?alt=media&token=a4670094-2a03-44ba-a655-9bcb45d7a167"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-101302.png?alt=media&token=78dacf84-ec23-4aac-b44a-4fd1faaa3cf4"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-101318.png?alt=media&token=797147f2-d100-4602-bc50-f34ac91cbe67"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-101411.png?alt=media&token=7324f018-af53-4b41-9257-99971323367a"
    width="220" height="475">
</div>

Al inicio, podrás visitar las `reseñas` o escanear el QR de la lista de espera.
<br>
Una vez escaneado el QR, pasaras a fila de espera, de la cual podrás **salir** en cualquier momento.
<br>
Cuando el `Metre` le asigne una mesa se le mostrará cúal es, junto con un boton para escanear la mesa y asignarse a ella.
<br>
Ya asignado, puede comenzar a:
- Ver el menú (Redirige a `Menu` con queryparam **'buy=false'**)
- Hacer su pedido con `Hacer pedido`(Redirige a `Menu` con queryparam **'buy=true'**) , disponible siempre que no haya un pedido activo.
- Ingresar al `Chat` de la mesa para hacer cualquier consulta.

### Seguimiento del pedido:
<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-110028.png?alt=media&token=2b1fbe20-1aef-4c04-88a9-e79b0bb93d54"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-101943.png?alt=media&token=9f0c6b20-e53e-46cd-b7e3-ec75a05b6bf5"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-102159.png?alt=media&token=c313bbee-b7cf-4d77-9ad3-77074d30d4a8"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-102250.png?alt=media&token=7435f46f-bc6f-4250-9139-49ec0e38b5de"
    width="220" height="475">
</div>

Al escanear el QR de la mesa se desplegará un modal con las opciones:
- `Juegos` para ir a los juegos y acceder a descuentos.
- `Hacer reseña` para completar una reseña sobre nuestra atención.
- `Ver reseñas` para visitar la pagina de reseñas, se muestra cuando ya completaste tu reseña.
- `Ver pedido` para hacer el seguimiento del **pedido activo**
- `Pedir cuenta` para pagar la cuenta, se muestra cuando **no hay ningun pedido activo**.

Puedes ver en todo momento el estado de tu pedido activo, al `recibir el pedido` este pasa a la `cuenta de la mesa` por lo que deja de haber un pedido activo.

### Pedir cuenta:
<div align="center">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-110507.png?alt=media&token=2f0e5a5f-0d68-4cf8-b4cc-54d322131987"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-102418.png?alt=media&token=299b80a1-eac3-49c4-92a3-cc0b5b17f867"
    width="220" height="475">
  <img
    src="https://firebasestorage.googleapis.com/v0/b/parcial-recursante.appspot.com/o/utils%2Fscreenshots%2Fcliente%2FScreenshot_20231118-102441.png?alt=media&token=6bc8936e-df92-40eb-8d52-42a2cac1982d"
    width="220" height="475">
</div>

Al pedir la cuenta se despliega un modal en el que verás todo lo consumido durante tu estadia, además de poder calcular la **propina** de acuerdo a la atención brindada.
<br>
Una vez hecho el pago, pasarás a esperar a que el `Mozo` **confirme el pago**.









